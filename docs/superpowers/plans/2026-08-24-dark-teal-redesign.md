...
n = p.count('card--blog')
print("blog cards:", n, "(expected 4 locally: 3 tracked + 1 untracked draft)")
print("kickers:", p.count('class="kicker"'), "(should equal card count)")
print("h1 is 'Blog':", '>Blog<' in p)
bodies = len(re.findall(r'class="card__body"', p))
print("card bodies:", bodies, "(expected 3 — the draft has no excerpt)")
PY
```

Expected: 4 cards locally and 3 bodies. In production it is 3 and 3. If you see 3 cards locally, the untracked draft was deleted — restore it, it is out of scope.
